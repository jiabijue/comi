...
class PositionalEmbedding(nn.Module):
    def __init__(self, demb):
        ...
        inv_freq = 1 / (10000 ** (torch.arange(0.0, demb, 2.0) / demb))  # [d_emb / 2, ]
        self.register_buffer('inv_freq', inv_freq)

    def forward(self, pos_seq, bsz=None):
        sinusoid_inp = torch.ger(pos_seq, self.inv_freq)  # [seq_len, d_emb / 2]
        pos_emb = torch.cat([sinusoid_inp.sin(), sinusoid_inp.cos()], dim=-1)  # [seq_len, d_emb]

        if bsz is not None:
            return pos_emb[:, None, :].expand(-1, bsz, -1)  # [seq_len, bsz, d_emb]
        else:
            return pos_emb[:, None, :]  # [seq_len, 1, d_emb]
...
class MultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, pre_lnorm=False):
        ...
        self.pre_lnorm = pre_lnorm  # pre-layer normalization
...
# Decoder with absolute embeddings, attn_type in [2, 3]
...
# Decoder with learnable embeddings, attn_type=1
...
# The default Decoder (Transformer-XL Decoder), attn_type=0
...
class MemTransformerLM(nn.Module):
    def __init__(self, n_token, n_layer, d_model, n_head,
                 d_head, d_inner, dropout, dropatt,
                 d_embed=None, cutoffs=[], tie_projs=[False],
                 tie_weight=True, div_val=1,
                 pre_lnorm=False, tgt_len=None,
                 ext_len=None, mem_len=None,
                 same_length=False, attn_type=0,
                 clamp_len=-1, sample_softmax=-1):
        super(MemTransformerLM, self).__init__()
        self.n_token = n_token  # vocabulary size
        ...
        self.tgt_len = tgt_len  # jbj:?
        self.mem_len = mem_len
        self.ext_len = ext_len  # jbj:?
        self.max_klen = tgt_len + ext_len + mem_len  # jbj:?
    ...
    def _create_params(self):
        if self.attn_type == 0:  # default attention
            ...
            self.r_w_bias = nn.Parameter(torch.Tensor(self.n_head, self.d_head))  # jbj: torch.Tensor与torch.empty效果一样？
    ...
    def compute_random_loss(self, data, target):
        seq_len, bsz = data.size()
        rand_pred = torch.randn(seq_len*bsz, self.n_token)  # rand()
        return F.cross_entropy(rand_pred, target.view(-1).to(torch.device('cpu')))
    ...
    for div_val in [1, 2]:
        for d_embed in [200, 100]:
            model = MemTransformerLM(args.n_token, args.n_layer, args.d_model, args.n_head,
                                     ...
                                     d_embed=d_embed, cutoffs=cutoffs, tie_projs=tie_projs,
                                     tie_weight=True, div_val=div_val,
                                     pre_lnorm=True, tgt_len=args.tgt_len,
